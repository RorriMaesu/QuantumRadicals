...
// electron gyromagnetic ratio in MHz / mT (more precise value)
const gamma_e = 28.024;

// Compute Δω (effective singlet-triplet splitting in MHz)
function deltaOmega({B,A,J,D}) {
  const wZ = gamma_e * B;     // Zeeman splitting in MHz
  return Math.sqrt((A/2)**2 + (J+D-wZ)**2);
}

// Time-domain singlet probability (analytical S ↔ T oscillation with decoherence γ)
export function singletProb(t, p=params){
  const dw = deltaOmega(p);
  return 0.5*(1 + Math.exp(-p.gamma*t)*Math.cos(2*pi*dw*t));
}

// Polar yield integral Φ_S(θ) for 0-π (fast numeric)
export function yieldAtAngle(theta, p=params){
  // simple orientation via A_z = A cosθ
  const Aeff = Math.abs(p.A*Math.cos(theta));
